...
class GeneratorValidator:
    ...
    def check_nearly_sorted(n: int, swaps: int, *, seed=None) -> list|None:
        if n < 0:
            raise IncorrectInputError(f"n must be non-negative. But recieved n = {n}")
        if swaps < 0:
            raise IncorrectInputError(f"swaps must be non-negative. But recieved swaps = {swaps}")
        if swaps > n:
            raise IncorrectInputError(f"swaps must be less than or equal to n. But recieved swaps = {swaps} > n = {n}")
        if n == 0:
            return []
